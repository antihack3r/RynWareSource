...
public class Shaders
{
        ...
        for (final Object s2 : set) {
            ...
            if (s3.startsWith(s)) {
                ...
                if (i < 0) {
                    ...
                    final CustomTexture customtexture = loadCustomTexture(i, s5);
    ...
    private static CustomTexture loadCustomTexture(final int index, String path) {
        if (path == null) {
            return null;
        }
        path = path.trim();
        ...
        try {
            ...
            final CustomTexture customtexture = new CustomTexture(index, s, simpleshadertexture);
            ...
            try {
                ...
                while (true) {
                    ...
                    if (shaderline.isUniform()) {
                        ...
                        int k1;
                        if ((k1 = ShaderParser.getShadowDepthIndex(s2)) >= 0) {
                            Shaders.usedShadowDepthBuffers = Math.max(Shaders.usedShadowDepthBuffers, k1 + 1);
                        }
                        else if ((k1 = ShaderParser.getShadowColorIndex(s2)) >= 0) {
                            Shaders.usedShadowColorBuffers = Math.max(Shaders.usedShadowColorBuffers, k1 + 1);
                        }
                        else if ((k1 = ShaderParser.getDepthIndex(s2)) >= 0) {
                            Shaders.usedDepthBuffers = Math.max(Shaders.usedDepthBuffers, k1 + 1);
                        }
                        else if (s2.equals("gdepth") && Shaders.gbuffersFormat[1] == 6408) {
                            Shaders.gbuffersFormat[1] = 34836;
                        }
                        else if ((k1 = ShaderParser.getColorIndex(s2)) >= 0) {
                            Shaders.usedColorBuffers = Math.max(Shaders.usedColorBuffers, k1 + 1);
                        }
                        else {
                                ...
                                if (shaderline.isConstFloat("shadowDistanceRenderMul")) {
                                    ...
                                    if (!shaderline.isConstFloat("drynessHalflife") && !shaderline.isComment("DRYNESSHL")) {
                                        if (shaderline.isConstFloat("eyeBrightnessHalflife")) {
                                            ...
                                            final int j = getBufferIndexFromString(s3);
                                            final int l = getTextureFormatFromString(s4);
                                            if (j < 0 || l == 0) {
                                                continue;
                                            }
                                            Shaders.gbuffersFormat[j] = l;
                                            ...
                                            final int j2 = getBufferIndexFromString(s5);
                                            if (j2 < 0) {
                                                continue;
                                            }
                                            Shaders.gbuffersClear[j2] = false;
                                            ...
                                            final String s6 = StrUtils.removeSuffix(shaderline.getName(), "MipmapEnabled");
                                            final int m = getBufferIndexFromString(s6);
                                            if (m < 0) {
                                                continue;
                                            }
                                            ...
                                            if (!shaderline.isComment("DRAWBUFFERS")) {
                                                continue;
                                            }
                                            final String s7 = shaderline.getValue();
                                            if (ShaderParser.isValidDrawBuffers(s7)) {
                                                Shaders.newDrawBufSetting = s7;
                                            }
                                            else {
    ...
    private static void checkWorldChanged(final World worldd) {
        if (Shaders.currentWorld != worldd) {
            ...
            if (world != null && worldd != null) {
                ...
                final int j = worldd.provider.getDimensionType().getId();

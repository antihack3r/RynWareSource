...
public class CompressedNBTType extends Type<CompoundTag>
{
    ...
    public CompoundTag read(final ByteBuf buffer) throws IOException {
        ...
        try (final GZIPInputStream gzipStream = new GZIPInputStream((InputStream)new ByteBufInputStream(compressed))) {
    ...
    public void write(final ByteBuf buffer, final CompoundTag nbt) throws Exception {
        ...
        try {
            try (final GZIPOutputStream gzipStream = new GZIPOutputStream((OutputStream)new ByteBufOutputStream(compressedBuf))) {

...
import org.rynware.client.event.events.impl.player.EventUpdateLiving;
...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public void onUpdate() {
        this.noClip = (Main.instance.featureManager.getFeature(NoClip.class).isEnabled() || Main.instance.featureManager.getFeature(FreeCam.class).isEnabled() || this.isSpectator());
        final EventUpdateLiving event = new EventUpdateLiving();
    ...
    public float getDigSpeed(final IBlockState state, final ItemStack itemstack) {
        float f = itemstack.getStrVsBlock(state);
        if (f > 1.0f) {
            final int i = EnchantmentHelper.getEnchantmentLevel(Enchantments.EFFICIENCY, itemstack);
            if (i > 0 && !itemstack.func_190926_b()) {
                f += i * i + 1;
            }
        }
        if (this.isPotionActive(MobEffects.HASTE)) {
            f = 1.0f + (this.getActivePotionEffect(MobEffects.HASTE).getAmplifier() + 1);
        }
    ...
    public String getName() {
        ...
        if (this == Minecraft.getMinecraft().player && FakeNameCommand.canChange) {
            ...
            str = ((Main.instance.featureManager.getFeature(NameProtect.class).isEnabled() && NameProtect.myName.getBoolValue() && this == Minecraft.getMinecraft().player) ? "Protected" : this.gameProfile.getName());
        }
        return str;
    }
    ...
    public boolean isPushedByWater() {
        return (!Main.instance.featureManager.getFeature(NoPush.class).isEnabled() || !NoPush.water.getBoolValue()) && !this.capabilities.isFlying;
    }
    ...
    public float getEyeHeight() {
        ...
        if (this.isPlayerSleeping()) {
            ...
            if (this.isElytraFlying() || this.height == 0.6f) {
                if (Main.instance.featureManager.getFeature(Flight.class).isEnabled() && Flight.flyMode.currentMode.equals("Matrix Elytra")) {
                    f = 1.62f;
                }
                else {
                    f = 0.4f;

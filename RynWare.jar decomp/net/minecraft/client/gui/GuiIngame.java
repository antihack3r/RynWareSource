...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(final float partialTicks) {
        ...
        final EventRender2D event = new EventRender2D(scaledresolution);
        ...
        if (this.recordPlayingUpFor > 0) {
            ...
            if (l1 > 8) {
                ...
                fontrenderer.drawString(this.recordPlaying, (float)(-fontrenderer.getStringWidth(this.recordPlaying) / 2), -4.0f, m + (l1 << 24 & 0xFF000000));
                GlStateManager.disableBlend();
        ...
        if (this.titlesTimer > 0) {
            ...
            if (i2 > 8) {
                ...
                fontrenderer.drawString(this.displayedTitle, (float)(-fontrenderer.getStringWidth(this.displayedTitle) / 2), -10.0, 0xFFFFFF | j3, true);
                GlStateManager.popMatrix();
                GlStateManager.pushMatrix();
                GlStateManager.scale(2.0f, 2.0f, 2.0f);
                fontrenderer.drawString(this.displayedSubTitle, (float)(-fontrenderer.getStringWidth(this.displayedSubTitle) / 2), 5.0, 0xFFFFFF | j3, true);
                GlStateManager.popMatrix();
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
        ...
        if (scoreobjective2 != null) {
            final EventRenderScoreboard e = new EventRenderScoreboard(EventType.PRE);
            EventManager.call(e);
            if (!Main.instance.featureManager.getFeature(ScoreboardFeatures.class).isEnabled() || !ScoreboardFeatures.noScore.getBoolValue()) {
                this.renderScoreboard(scoreobjective2, scaledresolution);
            }
            final EventRenderScoreboard event2 = new EventRenderScoreboard(EventType.POST);
        ...
        if ((this.mc.gameSettings.keyBindPlayerList.isKeyDown() || this.overlayPlayerList.addition > 0.0f) && (!this.mc.isIntegratedServerRunning() || this.mc.player.connection.getPlayerInfoMap().size() > 1 || scoreobjective2 != null)) {
    ...
    protected void renderPotionEffects(final ScaledResolution resolution) {
        if ((Main.instance.featureManager.getFeature(Hud.class).isEnabled() && Hud.potions.getBoolValue()) || Main.instance.featureManager.getFeature(FeatureList.class).isEnabled()) {
            return;
        }
    ...
    protected void renderHotbar(final ScaledResolution sr, final float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            RenderUtils.drawSmoothRect(i - 91, sr.getScaledHeight() - 24, sr.getScaledWidth() / 2 + 91, sr.getScaledHeight() - 2, new Color(16, 15, 15, 85).getRGB());
            RenderUtils.drawBlur(5.0f, () -> RenderUtils.drawSmoothRect(i - 91, sr.getScaledHeight() - 24, sr.getScaledWidth() / 2 + 91, sr.getScaledHeight() - 2, new Color(16, 15, 15, 85).getRGB()));
            ...
            for (int l = 0; l < 9; ++l) {
                ...
                if (this.mc.player.inventory.currentItem == l) {
                    RenderUtils.drawSmoothRect(i2 - 1, sr.getScaledHeight() - 21, i2 + 17, sr.getScaledHeight() - 3, RenderUtils.injectAlpha(ClientHelper.getClientColor(), 145).getRGB());
                }
    ...
    public void renderExpBar(final ScaledResolution scaledRes, final int x) {
        ...
        if (this.mc.player.experienceLevel > 0) {
            ...
            final String s = "" + this.mc.player.experienceLevel;
            final int k2 = (scaledRes.getScaledWidth() - this.getFontRenderer().getStringWidth(s)) / 2;
            final int i2 = scaledRes.getScaledHeight() - 31 - 4;
            this.getFontRenderer().drawString(s, (float)(k2 + 1), (float)i2, 0);
            this.getFontRenderer().drawString(s, (float)(k2 - 1), (float)i2, 0);
            this.getFontRenderer().drawString(s, (float)k2, (float)(i2 + 1), 0);
            this.getFontRenderer().drawString(s, (float)k2, (float)(i2 - 1), 0);
            this.getFontRenderer().drawString(s, (float)k2, (float)i2, j2);
            this.mc.mcProfiler.endSection();
        }
    }
    ...
    public void renderDemo(final ScaledResolution scaledRes) {
        ...
        this.getFontRenderer().drawStringWithShadow(s, (float)(scaledRes.getScaledWidth() - i - 10), 5.0, 16777215);
        this.mc.mcProfiler.endSection();
    }
    ...
    private void renderScoreboard(final ScoreObjective objective, final ScaledResolution scaledRes) {
        ...
        for (final Score score2 : collection) {
            ...
            if (Main.instance.featureManager.getFeature(NameProtect.class).isEnabled() && NameProtect.scoreboardSpoof.getBoolValue()) {
                this.getFontRenderer().drawString(s2.replace(Minecraft.getMinecraft().player.getName().substring(0, 2), TextFormatting.GOLD + "RynWare                                                                                                 "), (float)l1, (float)k2, 553648127);
            }
            else {
    ...
    private void renderPumpkinOverlay(final ScaledResolution scaledRes) {
        if (Main.instance.featureManager.getFeature(NoOverlay.class).isEnabled()) {
            return;
        }
        GlStateManager.disableDepth();

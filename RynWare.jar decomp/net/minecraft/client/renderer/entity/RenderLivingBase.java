...
import org.rynware.client.event.events.impl.render.EventRenderPlayerName;
...
public abstract class RenderLivingBase<T extends EntityLivingBase> extends Render<T>
{
    ...
    protected void renderModel(final T entitylivingbaseIn, final float limbSwing, final float limbSwingAmount, final float ageInTicks, final float netHeadYaw, final float headPitch, final float scaleFactor) {
        ...
        final String mode = Chams.chamsMode.getCurrentMode();
        final boolean chamsCheck = Main.instance.featureManager.getFeature(Chams.class).isEnabled() && entitylivingbaseIn instanceof EntityPlayer;
        final Color chamsColorValue = new Color(Chams.colorChams.getColorValue());
        final Color color = new Color(chamsColorValue.getRed(), chamsColorValue.getGreen(), chamsColorValue.getBlue());
        if (flag || flag2) {
            ...
            if (chamsCheck && mode.equalsIgnoreCase("Fill")) {
                ...
                if (!Chams.clientColor.getBoolValue()) {
                    GlStateManager.color(color.darker().getRed() / 255.0f, color.darker().darker().getGreen() / 255.0f, color.darker().getBlue() / 255.0f, Chams.chamsAlpha.getNumberValue());
                }
                else {
                    GlStateManager.color(ClientHelper.getClientColor().darker().getRed() / 255.0f, ClientHelper.getClientColor().darker().getGreen() / 255.0f, ClientHelper.getClientColor().darker().getBlue() / 255.0f, Chams.chamsAlpha.getNumberValue());
                }
                GL11.glDisable(2929);
            ...
            if (chamsCheck && mode.equalsIgnoreCase("Fill")) {
                ...
                if (!Chams.clientColor.getBoolValue()) {
                    GlStateManager.color(color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, Chams.chamsAlpha.getNumberValue());
                }
                else {
                    GlStateManager.color(ClientHelper.getClientColor().getRed() / 255.0f, ClientHelper.getClientColor().getGreen() / 255.0f, ClientHelper.getClientColor().getBlue() / 255.0f, Chams.chamsAlpha.getNumberValue());
                }
                this.mainModel.render(entitylivingbaseIn, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scaleFactor);
                GL11.glEnable(3553);
    ...
    public void renderName(final T entity, final double x, final double y, final double z) {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z)) {
            if (this.canRenderName(entity)) {
                ...
                if (d0 < f * f) {
                    final EventRenderPlayerName eventNameTag = new EventRenderPlayerName(entity, entity.getDisplayName().getFormattedText());

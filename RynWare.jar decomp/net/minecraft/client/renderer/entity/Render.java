...
public abstract class Render<T extends Entity> implements IEntityRenderer
{
    ...
    protected void renderLivingLabel(final T entityIn, String str, final double x, final double y, final double z, final int maxDistance) {
        if ((Main.instance.featureManager.getFeature(NameTags.class).isEnabled() && !AntiBot.isBotPlayer.contains(entityIn)) || Main.instance.featureManager.getFeature(NameTags.class).isEnabled()) {
            return;
        }
        ...
        if (d0 <= maxDistance * maxDistance) {
            ...
            if (Main.instance.featureManager.getFeature(NameProtect.class).isEnabled() && NameProtect.otherName.getBoolValue() && str.contains(entityIn.getName()) && !(entityIn instanceof EntityArmorStand)) {
                str = "Protected";
            }

...
public class RenderMinecart<T extends EntityMinecart> extends Render<T>
{
    ...
    public void doRender(final T entity, double x, double y, double z, float entityYaw, final float partialTicks) {
        ...
        if (vec3d != null) {
            ...
            x += vec3d.xCoord - d0;
            y += (vec3d2.yCoord + vec3d3.yCoord) / 2.0 - d2;
            z += vec3d.zCoord - d3;
            Vec3d vec3d4 = vec3d3.addVector(-vec3d2.xCoord, -vec3d2.yCoord, -vec3d2.zCoord);
            if (vec3d4.lengthVector() != 0.0) {
                vec3d4 = vec3d4.normalize();
                entityYaw = (float)(Math.atan2(vec3d4.zCoord, vec3d4.xCoord) * 180.0 / 3.141592653589793);
                f4 = (float)(Math.atan(vec3d4.yCoord) * 73.0);
            }
        }

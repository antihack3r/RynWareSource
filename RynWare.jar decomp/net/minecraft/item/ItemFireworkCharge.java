            ...
            for (final int i : aint) {
                ...
                for (int j = 0; j < ItemDye.DYE_COLORS.length; ++j) {
                    if (i == ItemDye.DYE_COLORS[j]) {
                        flag2 = true;
                        s += I18n.translateToLocal("item.fireworksCharge." + EnumDyeColor.byDyeDamage(j).getUnlocalizedName());
                        break;
                    }
                }
                if (!flag2) {
            ...
            for (final int l : aint2) {
                ...
                for (int k = 0; k < 16; ++k) {
                    if (l == ItemDye.DYE_COLORS[k]) {
                        flag4 = true;
                        s2 += I18n.translateToLocal("item.fireworksCharge." + EnumDyeColor.byDyeDamage(k).getUnlocalizedName());
                        break;
                    }
                }
                if (!flag4) {

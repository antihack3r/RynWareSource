        ...
        if (i > 4096L) {
            ...
            if (this.getIslandHeightValue(x, z, 1, 1) > 40.0f) {
                for (int j = this.rand.nextInt(5), k = 0; k < j; ++k) {
                    ...
                    final int j2 = this.worldObj.getHeight(blockpos.add(l, 0, i2)).getY();
                    if (j2 > 0) {
                        final int k2 = j2 - 1;
                        if (this.worldObj.isAirBlock(blockpos.add(l, k2 + 1, i2)) && this.worldObj.getBlockState(blockpos.add(l, k2, i2)).getBlock() == Blocks.END_STONE) {
                            BlockChorusFlower.generatePlant(this.worldObj, blockpos.add(l, k2 + 1, i2), this.rand, 8);
                        }
                    }
                }
                if (this.rand.nextInt(700) == 0) {
                    ...
                    final int j3 = this.worldObj.getHeight(blockpos.add(l2, 0, i3)).getY();
                    if (j3 > 0) {
                        final int k3 = j3 + 3 + this.rand.nextInt(7);
                        final BlockPos blockpos2 = blockpos.add(l2, k3, i3);
                        new WorldGenEndGateway().generate(this.worldObj, this.rand, blockpos2);

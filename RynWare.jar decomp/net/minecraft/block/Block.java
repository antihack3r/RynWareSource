...
public class Block
{
    ...
    public boolean canCollideCheck(final IBlockState state, final boolean hitIfLiquid) {
        for (final Block block : Objects.requireNonNull(NoInteract.getRightClickableBlocks())) {
            if (state.getBlock() == block) {
                if (!Main.instance.featureManager.getFeature(NoInteract.class).isEnabled()) {
                    continue;
                }
                return false;
            }
    ...
    public float getPlayerRelativeBlockHardness(final IBlockState state, final EntityPlayer player, final World worldIn, final BlockPos pos) {
        ...
        final AutoTool autoTool = (AutoTool)Main.instance.featureManager.getFeature(AutoTool.class);
        return player.canHarvestBlock(state) ? (player.getDigSpeed(state, (autoTool.isEnabled() && autoTool.silentSwitch.getBoolValue()) ? player.inventory.getStackInSlot(autoTool.itemIndex) : player.getHeldItemMainhand()) / f / 30.0f) : (player.getDigSpeed(state, (autoTool.isEnabled() && autoTool.silentSwitch.getBoolValue()) ? player.inventory.getStackInSlot(autoTool.itemIndex) : player.getHeldItemMainhand()) / f / 100.0f);
    }
